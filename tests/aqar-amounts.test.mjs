...
test("يفهم مليونًا وآلافًا متتالية حتى لو أغفل المعلن حرف الواو", () => {
  assert.equal(priceFrom("المطلوب 3 مليون 500 ألف"), 3_500_000);
});
...
test("لا يحوّل رقم عرض الشارع بعد كلمة الحدود إلى سعر في إعلان 6828963", () => {
  const html = `<h1>ورشة للبيع في مدينة الرياض، حي النور</h1><div>8,024,212 ﷼</div><div>استكشف خيارات التمويل</div><p>ثلاث ورش للبيع. الحدود جنوب: شارع عرض 25 متر. تفاصيل المساحات: الورشة الأولى 1,265.79 م²، الورشة الثانية 800 م²، الورشة الثالثة 800 م². إجمالي المساحة: 2,865.79 م².</p><h3>تفاصيل الإعلان</h3><div>رقم الإعلان 6828963</div>`;
...
test("لا يحوّل رقم عرض الشارع بعد كلمة الحدود إلى سعر في إعلان 6501536", () => {
  const html = `<h1>ورشة للبيع في مدينة الرياض</h1><div>8,000,000 ﷼</div><div>استكشف خيارات التمويل</div><p>ورشة للبيع في الصناعية القديمة. الحدود جنوب: شارع عرض 20 متر. المساحة 750 م².</p><h3>تفاصيل الإعلان</h3><div>رقم الإعلان 6501536</div>`;
  const listing = parseListing(html, "https://sa.aqar.fm/ورش-للبيع/الرياض/ورشة-6501536", "ورشة");
  assert.equal(listing.price, 8_000_000);
  assert.equal(listing.area, 750);
});

test("يبقي حقل الحد الصريح صالحًا بعد منع مطابقته لكلمة الحدود", () => {
  assert.equal(priceFrom("الحد 1,500,000"), 1_500_000);
});

test("لا يحوّل طول الحد الشمالي إلى سعر في الإعلان 6862905", () => {
  const html = `<h1>أرض للبيع في مدينة الرياض، حي المونسية</h1><div>18,750,000 ﷼</div><div>استكشف خيارات التمويل</div><p>أرض تجارية للبيع مساحة 2500م. الحد الشمالي بطول 50.36 م شارع التعدين 30م. الحد الشرقي بطول 50.36 م ممر شارع 15م. السعر 7500 للمتر.</p><h3>تفاصيل الإعلان</h3><div>المساحة 2500 م²</div><div>رقم الإعلان 6862905</div>`;
  const listing = parseListing(html, "https://sa.aqar.fm/أراضي-للبيع/الرياض/أرض-6862905", "أرض");
  assert.equal(listing.price, 18_750_000);
  assert.equal(listing.area, 2_500);
  assert.equal(listing.sqmPrice, 7_500);
});
...
test("لا يحوّل عدد المعارض بعد عنوان المساحات إلى مساحة في إعلان 6794603", () => {
  const html = `
    <h1>محل للبيع في شارع وادي الريحان، حي القادسية، مدينة الرياض</h1>
    <div>3,500,000 ﷼</div><div>استكشف خيارات التمويل</div>
    <p>المساحات:
      • 4 معارض بمساحة 58.46 م² لكل معرض
      • معرضان بمساحة 50 م² لكل معرض
      • مكتب بمساحة 12.5 م²
      • مكتب بمساحة 14.5 م²
      • إجمالي مساحة العقار: 649.8 م²
      • إجمالي مساحة مسطحات البناء أكثر من: 484 م²
    </p>
    <h3>تفاصيل الإعلان</h3><p>المساحة 649 م² رقم الإعلان 6794603</p>
  `;
  const listing = parseListing(html, "https://sa.aqar.fm/محلات-للبيع/الرياض/محل-6794603", "محل");
  assert.equal(listing.area, 649.8);
  assert.equal(listing.price, 3_500_000);
});
...
test("يحسب أبعاد الأرض مساحة ولا يلتقط الضلع كما في الإعلان 6452882", () => {
  const html = `
    <h1>أرض للبيع في مدينة رنية، حي امتداد الشمال</h1>
    <div>112,000 ريال</div>
    <div>استكشف خيارات التمويل</div>
    <p>ارض مساحتها 20×20 في محافظة رنية الواقعة في الجزء الشمالي</p>
    <h3>تفاصيل الإعلان</h3>
    <p>عرض الشارع 15م، المساحة 400م²، سعر المتر 280 ريال</p>
  `;
  const listing = parseListing(html, "https://sa.aqar.fm/أراضي-للبيع/رنية/حي-امتداد-الشمال/أرض-6452882", "أرض");
  assert.equal(listing.area, 400);
  assert.equal(listing.sqmPrice, 280);
  assert.ok(!listing.warnings.some(w => w.startsWith("المساحة مختلفة")));
});

test("يعتمد مساحة القطعة الواحدة من الملخص عندما تكون التفاصيل إجمالي قطعتين كما في 6452523", () => {
  const html = `
    <h1>أرض للبيع في مدينة الرياض</h1>
    <div>المساحة 240 م²</div><div>500,000 ريال</div><div>استكشف خيارات التمويل</div>
    <p>للبيع قطعتين أرض متجاورتين بإجمالي مساحة 480 م²</p>
    <h3>تفاصيل الإعلان</h3><p>المساحة 480 م²، رقم الإعلان 6452523</p>
  `;
  const listing = parseListing(html, "https://sa.aqar.fm/أراضي-للبيع/الرياض/أرض-6452523", "أرض");
  assert.equal(listing.area, 240);
});

test("يراجع السعر والمساحة وسعر المتر معًا كما في الإعلان 6679749", () => {
  const html = `
    <h1>أرض للبيع في مدينة الرياض</h1>
    <div>1,485,063 ﷼</div><div>المساحة 900 م²</div><div>استكشف خيارات التمويل</div>
    <p>أرض بمساحة إجمالية 2,700 م²</p>
    <h3>تفاصيل الإعلان</h3><p>المساحة 900 م²، سعر المتر 1,650.07 ريال، رقم الإعلان 6679749</p>
  `;
  const listing = parseListing(html, "https://sa.aqar.fm/أراضي-للبيع/الرياض/أرض-6679749", "أرض");
  assert.equal(listing.area, 900);
  assert.ok(Math.abs((listing.price || 0) - (listing.area || 0) * (listing.sqmPrice || 0)) < 1);
  assert.ok(Math.abs((listing.sqmPrice || 0) - 1_650.07) < .01);
});
...
test("يقرأ عمر العمارة وعدد غرفها من صيغ الإعلان 6732780", () => {
  const html = `<h1>عمارة للبيع في مدينة الدمام، حي الزهور</h1>
    <p>العمر: أكثر من 16 سنة.</p><p>التقسيم: 6 شقق + ملحق.</p>
    <h2>تفاصيل الإعلان</h2><p>عدد الغرف</p><p>26</p>`;
  const listing = parseListing(html, "https://sa.aqar.fm/عمائر-للبيع/الدمام/عمارة-6732780", "عمارة");
  assert.equal(listing.age, "أكثر من 16 سنوات");
  assert.equal(listing.totalRooms, 26);
});
...
test("لا يحوّل نسبة العائد إلى دخل ويقدم الدخل الحالي كما في الإعلان 6837497", () => {
  const html = `<h1>عمارة للبيع في مدينة الدمام، حي المزروعية</h1><div>2,000,000 ﷼</div><div>استكشف خيارات التمويل</div><p>فرصة استثمارية بدخل قائم وعائد يقارب 8%، الدخل الحالي: قرابة 160,000 ريال سنويًا، العائد الإجمالي الحالي: حوالي 8% سنويًا</p>`;
  const listing = parseListing(html, "https://sa.aqar.fm/عمائر-للبيع/الدمام/حي-المزروعية/عمارة-6837497", "عمارة");
  assert.equal(listing.income, 160_000);
  assert.equal(listing.yieldPct, 8);
});
...
test("يقرأ صيغة موجرة بالكامل ويوسع الدخل المختصر كما في الإعلان 6451401", () => {
  const html = `<h1>عمارة للبيع في مدينة الدمام، حي البادية</h1><p>للبيع عمارة تجارية موجرة بالكامل ب 265 السعر ب مليونين و 800</p>`;
  const listing = parseListing(html, "https://sa.aqar.fm/عمائر-للبيع/الدمام/حي-البادية/عمارة-6451401", "عمارة");
  assert.equal(listing.price, 2_800_000);
  assert.equal(listing.income, 265_000);
  assert.ok(Math.abs(listing.yieldPct - (265_000 / 2_800_000 * 100)) < 1e-9);

  const explicitRiyals = parseListing(`<h1>عمارة للبيع في الدمام</h1><p>السعر مليون ريال، الدخل السنوي 265 ريال</p>`, "https://sa.aqar.fm/عمارة-7654328", "عمارة");
  assert.equal(explicitRiyals.income, 265);
});

test("يقرأ صيغ دخل وبدخل ودخله العامة كما في الإعلان 6546485", () => {
  for (const wording of ["بدخل 110000 الف ريال سنوي", "دخل 110,000 ريال سنوي", "دخله السنوي 110 ألف ريال", "دخلها 110 ألف ريال سنوي"]) {
    const html = `<h1>عمارة للبيع في مدينة الدمام، حي المنار</h1><div>1,500,000 ريال</div><div>استكشف خيارات التمويل</div><p>العمارة مؤجرة ${wording}</p><h3>تفاصيل الإعلان</h3><div>رقم الإعلان 6546485</div>`;
    const listing = parseListing(html, "https://sa.aqar.fm/عمائر-للبيع/الدمام/عمارة-6546485", "عمارة");
    assert.equal(listing.income, 110_000, wording);
    assert.ok(Math.abs(listing.yieldPct - (110_000 / 1_500_000 * 100)) < 1e-9, wording);
  }
});

test("يوسع إيجار الدفعة المختصر اعتمادًا على السعر السنوي كما في الإعلان 6872478", () => {
  const html = `<h1>دور للإيجار في مدينة الرياض، حي الياسمين</h1><div>85,000 ﷼ / سنوي</div><div>استأجر الآن وأدفع لاحقًا ابتداءً من 7,580 ﷼ شهريا</div><button>استأجر الآن</button><p>دور للإيجار. السعر 85 دفعة 90 دفعتين. قابل للتفاوض</p><h3>تفاصيل الإعلان</h3><div>رقم الإعلان 6872478</div>`;
  const listing = parseListing(html, "https://sa.aqar.fm/دور-للإيجار/الرياض/دور-6872478", "دور");
  assert.equal(listing.price, 85_000);
});
