...
export async function PUT(req:Request){ const userId=await uid(); const id=Number(new URL(req.url).searchParams.get("id")); const p=await req.json() as {name?:string}; const name=p.name?.trim(); if(!id||!name)return Response.json({error:"بيانات غير صالحة."},{status:400}); const [row]=await getDb().update(savedProfiles).set({name,updatedAt:new Date().toISOString()}).where(and(eq(savedProfiles.id,id),eq(savedProfiles.userId,userId))).returning(); if(!row)return Response.json({error:"لم توجد الشروط المحفوظة."},{status:404}); return Response.json({profile:row}); }
