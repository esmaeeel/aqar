...
export default function Home(){
  ...
  async function search(){
    ...
    for(let i=0;i<allSources.length&&checked.size<clean.maxListings;i++){
      ...
      try{const r=await fetch("/api/search",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({filters:clean,...s,remaining:Math.min(20,clean.maxListings-checked.size),excludeListingIds:[...checked]})});const data=await r.json() as {error?:string;discovered?:number;checkedListingIds?:string[];results?:Listing[];warnings?:string[]};if(!r.ok)throw new Error(data.error||"تعذر البحث");discovered+=data.discovered||0;for(const id of data.checkedListingIds||[])checked.add(id);for(const item of data.results||[])found.set(item.listingId,item);if(data.warnings?.length)setWarnings(w=>[...w,...data.warnings!].slice(-20));setResults([...found.values()].sort((a,b)=>b.score-a.score||(b.yieldPct||0)-(a.yieldPct||0)));const blocked=data.warnings?.find(w=>/429|منع|تحقق|تسجيل دخول/.test(w));if(blocked){stopReason=blocked;stoppedAt=i;break}}catch(e){const m=e instanceof Error?e.message:"تعذر البحث";setWarnings(w=>[...w,m]);if(/429|منع|تحقق|تسجيل دخول/.test(m)){stopReason=m;stoppedAt=i;break}}
    }
  ...
  function exportCsv(){
    ...
    if(filters.propertyType==="عمارة")fields.push(["housingUnits","وحدة سكنية"],["totalRooms","إجمالي الغرف"]);
...
type ColumnKey="price"|"income"|"yieldPct"|"area"|"sqmPrice"|"count"|"housingUnits"|"totalRooms"|"meters"|"floors"|"street"|"density"|"age";
...
const DEFAULT_COLUMN_ORDER:ColumnKey[]=["price","income","yieldPct","area","sqmPrice","count","housingUnits","totalRooms","meters","floors","street","density","age"];
const COLUMN_ORDER_KEY="aqar-mobile-table-column-order-v5";
...
function Results({rows,roomMode,propertyType,purpose,cities,onSave,saveDisabled}:{rows:Listing[];roomMode:boolean;propertyType:string;purpose:Filters["purpose"];cities:string[];onSave:()=>void;saveDisabled:boolean}){
  ...
  ].filter(column=>(propertyType==="عمارة"||!["housingUnits","totalRooms"].includes(column.key))&&(propertyType==="أرض"||column.key!=="sqmPrice")&&(!rentalHousing||!["yieldPct","density"].includes(column.key)));
