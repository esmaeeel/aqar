...
export default function Home(){
  ...
  function loadProfile(profile:Profile){const saved=JSON.parse(profile.filtersJson) as Partial<Filters>&{minCount?:number};const savedPropertyType=saved.propertyType||defaults.propertyType,legacyCount=Number(saved.minCount)||0;const loaded={...defaults,...saved,minApartments:saved.minApartments??(savedPropertyType==="عام"||ROOM_TYPES.has(savedPropertyType)?0:legacyCount),minRooms:saved.minRooms??(savedPropertyType==="عام"||!ROOM_TYPES.has(savedPropertyType)?0:legacyCount)} as Filters;setFilters(loaded);setKeywordDraft(loaded.keywords.join("، "));setGeneralKeywordError(false);setContextProfileId(null);profileMenuRef.current?.removeAttribute("open");setMessage(`تم تحميل: ${profile.name}`)}
  async function renameProfile(profile:Profile){const name=prompt("المسمى الجديد لشروط البحث:",profile.name)?.trim();if(!name||name===profile.name){setContextProfileId(null);return}const r=await fetch(storedApiUrl(`/api/profiles?id=${profile.id}`),{method:"PUT",headers:storedHeaders(true),body:JSON.stringify({name})});if(r.ok){await loadProfiles();setContextProfileId(null);profileMenuRef.current?.removeAttribute("open");setMessage(`تم تعديل المسمى إلى: ${name}`)}else setMessage("تعذر تعديل مسمى شروط البحث.")}
  async function deleteProfile(profile:Profile){if(!confirm(`هل تريد حذف شروط البحث المحفوظة «${profile.name}»؟`))return;const r=await fetch(storedApiUrl(`/api/profiles?id=${profile.id}`),{method:"DELETE",headers:storedHeaders()});if(r.ok){await loadProfiles();setContextProfileId(null);profileMenuRef.current?.removeAttribute("open");setMessage(`حُذفت شروط البحث: ${profile.name}`)}else setMessage("تعذر حذف شروط البحث.")}
