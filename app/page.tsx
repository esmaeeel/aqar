...
export default function Home(){
  ...
  function clearFields(){setFilters({...defaults,locations:[{city:"",neighborhoods:[]}]});setKeywordDraft("");setGeneralKeywordError(false);setResultSaveFeedback(null);setMessage("تم مسح جميع حقول البحث.")}
  ...
  function loadProfile(profile:Profile){const saved=JSON.parse(profile.filtersJson) as Partial<SearchFilters>&{minCount?:number};const savedPropertyType=saved.propertyType||defaults.propertyType,legacyCount=Number(saved.minCount)||0;const loaded={...defaults,...saved,minApartments:saved.minApartments??(savedPropertyType==="عام"||ROOM_TYPES.has(savedPropertyType)?0:legacyCount),minRooms:saved.minRooms??(savedPropertyType==="عام"||!ROOM_TYPES.has(savedPropertyType)?0:legacyCount)} as SearchFilters;setFilters(loaded);setKeywordDraft(loaded.keywords.join("، "));setGeneralKeywordError(false);profileMenuRef.current?.removeAttribute("open");setMessage(`تم تحميل: ${profile.name}`)}
