...
export default function Home(){
  ...
  function update<K extends keyof Filters>(key:K,value:Filters[K]){setFilters(f=>({...f,[key]:value}));if(key==="propertyType"&&value!=="عام")setGeneralKeywordError(false)}
  ...
  function loadProfile(profile:Profile){const saved=JSON.parse(profile.filtersJson) as Partial<Filters>&{minCount?:number};const savedPropertyType=saved.propertyType||defaults.propertyType,legacyCount=Number(saved.minCount)||0;const loaded={...defaults,...saved,minApartments:saved.minApartments??(savedPropertyType==="عام"||ROOM_TYPES.has(savedPropertyType)?0:legacyCount),minRooms:saved.minRooms??(savedPropertyType==="عام"||!ROOM_TYPES.has(savedPropertyType)?0:legacyCount)} as Filters;setFilters(loaded);setKeywordDraft(loaded.keywords.join("، "));setGeneralKeywordError(false);profileMenuRef.current?.removeAttribute("open");setMessage(`تم تحميل: ${profile.name}`)}
  ...
  async function search(){
    ...
    if(!generalSearchHasRequiredKeywords(clean.propertyType,clean.keywords)){setGeneralKeywordError(true);setMessage("عند اختيار «عام»، اكتب نوع العقار أو وصفه في «كلمات للبحث» أولًا.");return}
