...
export default function Home(){
  ...
  const [trial,setTrial]=useState<TrialStatus|null>(null),[browserId,setBrowserId]=useState("");
  useEffect(()=>{const timer=setTimeout(()=>{setBrowserId(getDeviceId());try{const x=localStorage.getItem(LAST_FILTERS_KEY);if(x)setFilters({...defaults,...JSON.parse(x)})}catch{/* تجاهل بيانات محلية تالفة */}void loadProfiles();void loadTrialStatus()},0);return()=>clearTimeout(timer)},[]);
  ...
  async function copyBrowserId(){if(!browserId)return;try{await navigator.clipboard.writeText(browserId);setMessage("نُسخ معرّف هذا المتصفح. أرسله لتعيين حد خاص له.")}catch{window.prompt("انسخ معرّف هذا المتصفح:",browserId)}}
