  ...
  return <main dir="rtl">
    ...
    <section className="panel searchPanel"><div className="sectionHead"><div><h2>مواصفات البحث</h2><p>تبدأ الحقول فارغة عند كل فتح. يمكنك تحميل بحث محفوظ يدويًا.</p></div><div className="profileControls"><select defaultValue="" onChange={e=>loadProfile(e.target.value)}><option value="">اختر بحثًا محفوظًا</option>{profiles.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select><button className="ghost" onClick={saveProfile}>حفظ الشروط</button><button className="ghost" onClick={clearFields}>مسح الحقول</button></div></div>
      <div className="searchFields">
        ...
        <div className="formBlock descriptionBlock"><h3>البحث في الوصف</h3><div className="descriptionFields"><label>كلمات مطلوبة (بفواصل)<input value={keywordDraft} onChange={e=>changeKeywordDraft(e.target.value)} placeholder={filters.propertyType==="عام"?"مثل: مكيف، موقف، سيارة. أو نوع العقار: مزرعة فندق":"مثال: مكيفات، مدخل سيارة، صناعات"}/>{filters.propertyType!=="عام"&&<small>يمكن استخدام الفاصلة العربية «،» أو الإنجليزية «,». وتُقبل المسافات داخل العبارة مثل: مدخل سيارة.</small>}</label>
          <div className="limits"><label>أقصى إعلانات<input type="number" min="5" max="500" value={filters.maxListings} onChange={e=>update("maxListings",inputNumber(e.target.value))}/></label></div>
        </div></div>
      </div>
