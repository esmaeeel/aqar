...
type TrialStatus={limit:number;used:number;remaining:number;canStart:boolean;retryAfterSeconds:number;nextAllowedAt:number|null};
...
export default function Home(){
  ...
  const trialBlocked=trial?.remaining===0||cooldownSeconds>0;
  ...
  async function loadTrialStatus(){try{const r=await fetch("/api/trial",{cache:"no-store"});const data=await r.json() as TrialStatus;if(r.ok)setTrial(data)}catch{/* يتحقق الخادم مرة أخرى عند بدء البحث */}}
  ...
  async function search(){
    ...
    let trialToken="";
    try{const reservationResponse=await fetch("/api/trial",{method:"POST"});const reservation=await reservationResponse.json() as {token?:string|null;status:TrialStatus;error?:string};setTrial(reservation.status);setClock(Date.now());if(!reservationResponse.ok||!reservation.token){setMessage(reservation.error||"لا يمكن بدء بحث جديد الآن.");return}trialToken=reservation.token}catch{setMessage("تعذر التحقق من المحاولات التجريبية. حاول مرة أخرى.");return}
    ...
    for(let i=0;i<allSources.length&&checked.size<clean.maxListings;i++){
      ...
      try{const r=await fetch("/api/search",{method:"POST",headers:{"content-type":"application/json","x-aqar-trial-token":trialToken},body:JSON.stringify({filters:clean,...s,remaining:Math.min(20,clean.maxListings-checked.size),excludeListingIds:[...checked]})});const data=await r.json() as {error?:string;discovered?:number;checkedListingIds?:string[];results?:Listing[];warnings?:string[]};if(!r.ok){if(r.status===401||r.status===403){stopReason=data.error||"انتهت صلاحية محاولة البحث.";stoppedAt=i;break}throw new Error(data.error||"تعذر البحث")}discovered+=data.discovered||0;for(const id of data.checkedListingIds||[])checked.add(id);for(const item of data.results||[])found.set(item.listingId,item);if(data.warnings?.length)setWarnings(w=>[...w,...data.warnings!].slice(-20));setResults([...found.values()].sort((a,b)=>b.score-a.score||(b.yieldPct||0)-(a.yieldPct||0)));const blocked=data.warnings?.find(w=>/429|منع|تحقق|تسجيل دخول/.test(w));if(blocked){stopReason=blocked;stoppedAt=i;break}}catch(e){const m=e instanceof Error?e.message:"تعذر البحث";setWarnings(w=>[...w,m]);if(/429|منع|تحقق|تسجيل دخول/.test(m)){stopReason=m;stoppedAt=i;break}}
    }
  ...
  return <main dir="rtl">
    ...
    <section className="panel"><div className="sectionHead"><div><h2>مواصفات البحث</h2><p>تُحفظ آخر مواصفات تلقائيًا على هذا الجوال فقط.</p></div><div className="inline"><select defaultValue="" onChange={e=>loadProfile(e.target.value)}><option value="">اختر بحثًا محفوظًا</option>{profiles.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select><button className="ghost" onClick={saveProfile}>حفظ الشروط</button><button className="ghost" onClick={clearFields}>مسح الحقول</button></div></div>
      ...
      <div className="trialNotice" aria-live="polite"><strong>التجربة المشتركة</strong><span>{trial?`متبقّي ${trial.remaining} من ${trial.limit} عملية بحث`:`الحد الإجمالي 100 عملية بحث`}</span>{trial?.remaining===0?<span>انتهت المحاولات التجريبية.</span>:cooldownSeconds>0?<span>البحث التالي متاح بعد {Math.ceil(cooldownSeconds/60)} دقيقة.</span>:<span>يمكن بدء بحث جديد الآن.</span>}</div>
      <div className="run"><button className="primary" disabled={busy||trialBlocked} onClick={search}>{busy?"جارٍ البحث…":trial?.remaining===0?"انتهت التجربة":cooldownSeconds>0?`انتظر ${Math.ceil(cooldownSeconds/60)} دقيقة`:"ابدأ البحث في عقار"}</button></div>{busy&&<progress value={progress} max="100"/>}<div className="status">{message}</div>{warnings.length>0&&<details className="warnings"><summary>ملاحظات أثناء القراءة ({warnings.length})</summary>{warnings.map((w,i)=><p key={i}>{w}</p>)}</details>}
    </section>
