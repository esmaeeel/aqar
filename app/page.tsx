...
const defaults: SearchFilters = { propertyType:"عام",commercialOnly:false,purpose:"sale",locations:[{city:"الرياض",neighborhoods:[]}],keywords:[],mode:"strict",maxPages:2,maxListings:200,priceMin:0,priceMax:0,yieldMin:0,minMeters:0,minApartments:0,minRooms:0,minCommercialShops:0,minFloors:0,minStreet:0,areaMin:0,areaMax:0,minAge:0,maxAge:0,minDensity:0,sqmMin:0,sqmMax:0 };
...
export default function Home(){
  ...
  function clearFields(){setFilters({...defaults,locations:[{city:"",neighborhoods:[]}]});setKeywordDraft("");setGeneralKeywordError(false);setResults([]);setResultSaveFeedback(null);setContextProfileId(null);setMessage("تم مسح جميع حقول البحث.")}
...
function LocationAutocomplete({location,index,onChange,onAdd,onRemove}:{location:Location;index:number;onChange:(index:number,key:"city"|"neighborhoods",value:string)=>void;onAdd?:()=>void;onRemove:()=>void}){
  ...
  return <div className={`location ${onAdd?"locationHasAdd":""}`}>
    <label className="autocomplete">المدينة<input value={location.city} role="combobox" aria-expanded={cityOpen&&cityMatches.length>0} aria-controls={citySuggestionsId} autoComplete="off" onFocus={()=>setCityOpen(true)} onBlur={finishCity} onChange={event=>{onChange(index,"city",event.target.value);setCityOpen(true)}} onKeyDown={event=>{if(event.key==="Enter"&&cityMatches[0]){event.preventDefault();selectCity(cityMatches[0])}else if(event.key==="Escape")setCityOpen(false)}} placeholder="ابدأ الكتابة: جد…"/>{cityOpen&&cityMatches.length>0&&<span id={citySuggestionsId} className="suggestions citySuggestions" role="listbox">{cityMatches.map(value=><button type="button" key={value} role="option" onMouseDown={event=>event.preventDefault()} onClick={()=>selectCity(value)}>{value}</button>)}</span>}</label>
