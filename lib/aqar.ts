...
export const CATEGORIES: Record<string, Record<string, string[]>> = {
  ...
  "مستودع": { sale: ["مستودعات-للبيع"], rent: ["مستودع-للإيجار"] },
  "ورشة": {
...
// يقبل «3 مليون و500 ألف» وكذلك الصيغة الشائعة بلا واو: «3 مليون 500 ألف».
// لا يسمح بكلمات فاصلة أخرى، حتى لا يضم مبلغًا مستقلًا إلى السعر.
const amount = `([\\d][\\d,.]*|(?:مليون(?:ين|ان)?|الف|ألف)(?!\\w))[^\\S\\n]*(مليون(?:ين|ان)?|الف|ألف)?(?:\\s*(?:(?<![ء-يA-Za-z_])و(?![ء-يA-Za-z_])\\s*|(?<=\\s))([\\d][\\d,.]*)[^\\S\\n]*(مليون(?:ين|ان)?|الف|ألف)?)?`;
const gap = `[^\\d\\n]{0,40}?(?:\\n[^\\d\\n]{0,24}?)?`;
const incomeStopLabels = `السعر(?:\\s+المطلوب)?|سعر\\s+(?:البيع|العقار)|المطلوب|الحد|قيمة\\s+العقار|العائد|نسبة\\s+(?:العائد|الدخل)`;
...
export function parseListing(html: string, url: string, propertyType: string): Listing {
  ...
  const annualPatterns = [
    incomeLabeled(`الدخل\\s+السنوي(?:\\s+الحالي)?|الدخل\\s+الحالي|الايجار\\s+السنوي|الإيجار\\s+السنوي|صافي\\s+الدخل|الدخل\\s+(?:الصافي|الإجمالي|الاجمالي)`),
    incomeLabeled(`الدخل|المدخول|مدخول|(?:ب)?دخل(?:ه|ها)?(?:\\s+السنوي)?`),
...
export function evaluate(item: Listing, f: Filters) {
  ...
  const openEndedAge = /اكثر\s+من|فوق/.test(normalizedAge);
  const minAgeValue = !Number.isFinite(parsedAge) ? null
    : openEndedAge && minAge > parsedAge ? null
    : openEndedAge ? parsedAge + 0.001
    : parsedAge;
  const maxAgeValue = !Number.isFinite(parsedAge) ? null
    : openEndedAge && maxAge > parsedAge ? null
    : openEndedAge ? parsedAge + 0.001
    : parsedAge;
